...
class VRToRobotConverter(Node):
    def __init__(self):
        ...
        self.declare_parameter('position_scale', 0.3)  # hand motion scale: 1m hand -> 0.3m robot
        ...
        self.gripper_state = np.array([
            self.index_button_pressed,
            self.gripper_speed,
            self.gripper_force,
            self.epsilon_inner,
            self.epsilon_outer,
            self.gripper_grasp_width,
        ])  # position m, speed m/s, force N
    ...
    def parse_vr_binary(self, data, addr=None):
        """Parse 80-byte binary pose: 10 doubles (x,y,z, qx,qy,qz,qw, press_index, pause_val, extra) little-endian"""
        try:
            if len(data) != 80:
            ...
            # Transform VR coordinates to robot coordinates (same mapping as text path)
            ...
            # Store as float (0.0/1.0) so formatting works consistently later
    ...
    def parse_vr_text(self, text: str):
        ...
        for line in text.splitlines():
            ...
            # Accept common encodings: closed/open, true/false, 1/0.
            if 'fist' in label:
                fist_text = line.lower()
                fist_state = None

                if any(token in fist_text for token in ('closed', 'close', 'true', 'pressed', 'down')):
                    fist_state = 1.0
                elif any(token in fist_text for token in ('open', 'false', 'released', 'up')):
                    fist_state = 0.0
                else:
                    numeric_vals = []
                    for p in parts[1:]:
                    ...
                    if numeric_vals:
                        fist_state = 1.0 if numeric_vals[0] > 0.5 else 0.0
                    elif re.search(r'(^|\\D)1(\\D|$)', fist_text):
                        fist_state = 1.0
                    elif re.search(r'(^|\\D)0(\\D|$)', fist_text):
                        fist_state = 0.0

                if fist_state is not None:
                    self.pause_button_pressed = fist_state
                continue
    ...
    def control_loop(self):
        ...
        try:
            ...
            if self.is_paused:
                ...
                vr_pos_delta = (self.current_vr_pose['position'] - self.initial_vr_pose['position']) * self.position_scale + self.paused_vr_pose['position']
    ...
    def send_robot_command(self, position, orientation, gripper_state):
        ...
        try:
            # Send absolute pose: position (x,y,z) and orientation (qx,qy,qz,qw) and gripper state
            # (button_pressed, speed, force, epsilon_inner, epsilon_outer, width)
            message = (
                f"{position[0]:.6f} {position[1]:.6f} {position[2]:.6f} "
                f"{orientation[0]:.6f} {orientation[1]:.6f} {orientation[2]:.6f} {orientation[3]:.6f} "
                f"{gripper_state[0]:.6f} {gripper_state[1]:.6f} {gripper_state[2]:.6f} "
                f"{gripper_state[3]:.6f} {gripper_state[4]:.6f} {gripper_state[5]:.6f}"
            )
