...
ose: 10 doubles
    ...
    def control_loop(self):
        """Main control loop - hold-to-clutch version:
        hold pause button -> pause robot
        release pause button -> resume with re-clutch
        """
        ...
        try:
            ...
            if not self.control_started:
                ...
                if self.current_vr_pose is not None:
                    self.initial_vr_pose = {
                        'position': self.current_vr_pose['position'].copy(),
                        'orientation': self.current_vr_pose['orientation'].copy()
                    }

                # Reset accumulated offset
                self.paused_vr_pose['position'] = np.array([0.0, 0.0, 0.0], dtype=np.float64)
                self.paused_vr_pose['orientation'] = np.array([0.0, 0.0, 0.0, 1.0], dtype=np.float64)

                self.smoothed_position = np.array([0.0, 0.0, 0.0], dtype=np.float64)
                self.smoothed_orientation = np.array([0.0, 0.0, 0.0, 1.0], dtype=np.float64)

                self.get_logger().info("Control AUTO-STARTED. Hold left fist to clutch, release to resume.")

            # =========================================================
            # HOLD-TO-CLUTCH LOGIC
            # =========================================================

            # Case 1: button held down -> enter pause if not already paused
            if is_button_down and not self.is_paused:
                self.is_paused = True

                # Freeze current target pose
                self.paused_vr_pose['position'] = self.target_position.copy()
                self.paused_vr_pose['orientation'] = self.target_orientation.copy()

                self.get_logger().info("CLUTCH ON: robot paused. Move hand to a new pose and release to resume.")

            # Case 2: button released -> resume if previously paused
            elif (not is_button_down) and self.is_paused:
                self.is_paused = False

                # Re-clutch: define current hand pose as the new reference
                if self.current_vr_pose is not None:
                    self.initial_vr_pose = {
                        'position': self.current_vr_pose['position'].copy(),
                        'orientation': self.current_vr_pose['orientation'].copy()
                ...
                # Keep the robot at the frozen pose and continue from there
                self.smoothed_position = self.paused_vr_pose['position'].copy()
                self.smoothed_orientation = self.paused_vr_pose['orientation'].copy()

                self.get_logger().info("CLUTCH OFF: resumed control from new hand reference.")

            # =========================================================
            # Compute target pose
            # =========================================================
            if self.is_paused:
                # While clutch is held, robot target stays frozen
                self.target_position = self.paused_vr_pose['position'].copy()
                self.target_orientation = self.paused_vr_pose['orientation'].copy()

            else:
                # Relative translation from re-clutched reference pose
                vr_pos_delta = (
                    self.current_vr_pose['position']
                    - self.initial_vr_pose['position']
                    + self.paused_vr_pose['position']
                )

                # Position smoothing
                self.smoothed_position = (
                    self.smoothing_factor * self.smoothed_position
                    + (1.0 - self.smoothing_factor) * vr_pos_delta
                )

                # Relative rotation from re-clutched reference pose
                ...
                slerp_t = 1.0 - self.smoothing_factor
                ...
                self.target_orientation = self.smoothed_orientation.copy()

            # Update gripper state before sending
            self.gripper_state[0] = float(self.index_button_pressed)
            ...
            # Reset one-shot gripper press flag
            self.index_button_pressed = 0.0
