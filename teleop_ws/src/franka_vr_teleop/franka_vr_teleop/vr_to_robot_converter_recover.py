...
class VRToRobotConverter(Node):
    def __init__(self):
        ...
        # Parameters
        self.declare_parameter('vr_udp_ip', '192.168.0.214')
        ...
        self.gripper_force  = 40.0   # N
        self.epsilon_inner = 0.00  # m
        self.epsilon_outer = 0.03  # m
        self.gripper_grasp_width = 0.02 # m
        ...
        self.recover_button_pressed = 0.0
        self.last_recover_button_state = False
        self.recover_flag = 0.0  # 0.0 = normal, 1.0 = recover requested (sent to robot client)
    ...
    def receive_vr_data(self):
        ...
        while rclpy.ok():
            try:
                ...
                # 80 bytes = 10 doubles (with recover_button), 72 bytes = 9 doubles (legacy)
                if len(data) == 80:
                    self.parse_vr_binary_10(data, addr)
                elif len(data) == 72:
                    self.parse_vr_binary(data, addr)
                else:
                    print(f"WARNING: received data format is not binary pose (len={len(data)}). The sender might be incorrect")
    ...
    def _process_vr_values(self, x, y, z, qx, qy, qz, qw, press_index, pause_val, recover_val=0.0):
        """Common processing for VR pose data regardless of packet format."""
        ...
        self.recover_button_pressed = 1.0 if recover_val > 0.5 else 0.0

        if not self.vr_data_received:
            self.initial_vr_pose = self.current_vr_pose.copy()
            self.vr_data_received = True
            self.get_logger().info("Initial VR pose captured (binary)!")

        self.publish_vr_pose(robot_position, robot_quat)

    def parse_vr_binary_10(self, data, addr=None):
        """Parse 80-byte binary pose: 10 doubles (x,y,z, qx,qy,qz,qw, press_index, pause_val, recover_val) little-endian"""
        try:
            if len(data) != 80:
                self.get_logger().warn(f"Binary pose of unexpected length {len(data)}")
                return
            vals = struct.unpack('<10d', data)
            x, y, z, qx, qy, qz, qw, press_index, pause_val, recover_val = vals
            self._process_vr_values(x, y, z, qx, qy, qz, qw, press_index, pause_val, recover_val)
        except Exception as e:
            self.get_logger().error(f'Error parsing 80-byte binary VR pose: {str(e)}')

    def parse_vr_binary(self, data, addr=None):
        """Parse 72-byte binary pose: 9 doubles (x,y,z, qx,qy,qz,qw, press_index, pause_val) little-endian (legacy, no recover)"""
        try:
            if len(data) != 72:
                self.get_logger().warn(f"Binary pose of unexpected length {len(data)}")
                return
            vals = struct.unpack('<9d', data)
            x, y, z, qx, qy, qz, qw, press_index, pause_val = vals
            self._process_vr_values(x, y, z, qx, qy, qz, qw, press_index, pause_val, 0.0)
        except Exception as e:
            self.get_logger().error(f'Error parsing 72-byte binary VR pose: {str(e)}')
    ...
    def control_loop(self):
        ...
        try:
            ...
            # Detect recover button rising edge
            is_recover_down = self.recover_button_pressed > 0.5
            recover_triggered = is_recover_down and not self.last_recover_button_state
            self.last_recover_button_state = is_recover_down

            # Reset recover flag each tick (only set to 1.0 when actively triggered)
            self.recover_flag = 0.0
            ...
            if not self.control_started:
                if button_triggered:
                    ...
                    self.paused_vr_pose['orientation'] = np.array([0.0, 0.0, 0.0, 1.0]) # identity
                    
                    self.get_logger().info("Control ACTIVE.")
                else:
                    # Log periodically that we are waiting
                    # 100Hz -> every 300 ticks = 3 seconds
                    if self.command_counter % 300 == 0:
                        self.get_logger().info("Waiting for Button press to START control...")
                    return # DO NOT SEND COMMANDS YET
            ...
            # STATE: RUNNING (ACTIVE or PAUSED)
                ...
                # RECOVER: only when paused and recover button rising edge
                if self.is_paused and recover_triggered:
                    self.get_logger().info("RECOVER triggered. Commanding robot to initial pose.")
                    self.recover_flag = 1.0

                    # Reset internal state to match initial pose (zero offset from base)
                    self.paused_vr_pose['position'] = np.array([0.0, 0.0, 0.0])
                    self.paused_vr_pose['orientation'] = np.array([0.0, 0.0, 0.0, 1.0])  # identity
                    self.smoothed_position = np.array([0.0, 0.0, 0.0])
                    self.smoothed_orientation = np.array([0.0, 0.0, 0.0, 1.0])
                    ...
                    # Re-anchor hand reference so resume starts from zero delta
                    if self.current_vr_pose:
                        self.initial_vr_pose = self.current_vr_pose.copy()

            # Calculate Target Pose
            # If paused (including after recover), hold paused position
            if self.is_paused:
            ...
            # Send robot command (with recover flag appended)
            self.send_robot_command(self.target_position, self.target_orientation, self.gripper_state, self.recover_flag)
            ...
            if current_time - self.last_log_time >= self.log_interval:
                ...
                pause_info = f'Pause: {self.pause_button_pressed:.0f} | Recover: {self.recover_flag:.0f} | ({status_str})'
    ...
    def send_robot_command(self, position, orientation, gripper_state, recover_flag=0.0):
        """Send absolute pose command to robot via UDP.
        Format: x y z qx qy qz qw btn speed force eps_in eps_out width recover_flag
        (14 space-separated floats)
        """
        try:
