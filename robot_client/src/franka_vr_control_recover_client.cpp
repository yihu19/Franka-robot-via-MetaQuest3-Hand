...
struct VRCommand {
    ...
    double recover_flag = 0.0;          // 1.0 = recover to initial pose
        ...
        while (running_) {
            ...
            if (bytes_received > 0) {
                ...
                int parsed_count = sscanf(
                    buffer, "%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf",
                ...
                // Accept 13 (no recover flag, backward compat) or 14 values
                if (parsed_count >= 13) {
                    if (parsed_count < 14) cmd.recover_flag = 0.0;
...
private:
    ...
    void updateVRTargets(const VRCommand& cmd) {
        ...
        // ---- RECOVER: drive to initial robot pose ----
        if (cmd.recover_flag > 0.5) {
            ...
            // Reset filters so there is no smoothing lag when resuming normal control
            // Python sends (0,0,0) / identity during recover, matching initial_vr_position_
            filtered_vr_position_ = initial_vr_position_;
            filtered_vr_orientation_ = initial_vr_orientation_;
            return;
        }
    ...
    void run(const std::string& robot_ip,
        ...
        try {
            ...
            try {
                ...
                gripper_->move(0.06, 0.1);  // open gripper to a known state
            ...
            if (bidexhand_)
                ...
                // use [ 0.91772954  0.21037714 -0.27950428 -1.91012711  0.12014166  2.05900554 1.36743511]
                // Egg Boiler
                // q_goal = {{0.91772954, 0.21037714, -0.27950428, -1.91012711, 0.12014166, 2.05900554, -1.74129379}};
                
                
                // USB insert and pull out
                // use [ 1.12993788  0.17919591  0.27030564 -1.32822222 -0.0159883   1.56642192 -0.16459456]
                // q_goal = {{1.12993788, 0.17919591, 0.27030564, -1.32822222, -0.0159883, 1.56642192, -0.16459456}};

                // twist connector
                // use [ 0.94543768, 0.44071563, -0.29207247, -2.01221347, 0.23880782, 2.35466729, -1.81749505]
                q_goal = {{0.94543768, 0.44071563, -0.29207247, -2.01221347, 0.23880782, 2.35466729, -1.81749505}};


                // ram pull out
                // q_goal = {{1.05287561, -0.04943428, -0.35114074, -2.00461878, -0.03054411, 1.96707304, -0.04502555}};
