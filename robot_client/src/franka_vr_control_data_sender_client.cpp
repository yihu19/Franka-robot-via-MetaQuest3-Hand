...
// Single-producer single-consumer ring buffer sender that sends JSON lines via UDP
class RobotStateSender {
public:
    explicit RobotStateSender(const std::string &target_ip, int target_port, size_t capacity_pow2 = 16384)
        : target_ip_(target_ip), target_port_(target_port)
    {
        ...
        if (inet_pton(AF_INET, target_ip_.c_str(), &servaddr_.sin_addr) <= 0) {
    ...
    void writerLoop() {
        while (running_.load()) {
            ...
            if (sockfd_ >= 0) {
                ...
                sendto(sockfd_, msg.c_str(), msg.length(), 0, (const struct sockaddr *)&servaddr_, sizeof(servaddr_));
            }
        }
    }
...
private:
    ...
    // RT callback posts target + joints at ~200 Hz; IK thread solves and
    // writes the result back. RT callback reads with try_lock, falls back
    // to the cached solution when the solver is busy.
    // -----------------------------------------------------------------------
    struct IKInput {
...
public:
    void run(const std::string &robot_ip, const std::string &receiver_ip = "127.0.0.1", int receiver_port = 9091)
    {
        try
        {
            ...
            // Start sender (sends UDP packets asynchronously to avoid blocking the control loop)
...
private:
    // -----------------------------------------------------------------------
    // ikThreadFunc – polls ik_input_ready_ at ~2 kHz (500 µs sleep).
    // Solves IK and writes result to ik_output_. No syscall from RT callback.
    // -----------------------------------------------------------------------
    void ikThreadFunc()
    {
    ...
    void runVRControl(franka::Robot &robot)
    {
        ...
        {
            // try_lock: if the network thread is writing, reuse previous command
            ...
            // If try_lock fails cmd.has_valid_data == false → updateVRTargets returns early.
            updateVRTargets(cmd);
            ...
            // --- Debug printout (every 1 s) ---
            if (debug_counter % 1000 == 0) {
                std::cout << "IK: " << (cached_ik_valid_ ? "\033[32msuccess\033[0m" : "\033[31mfail\033[0m") << " | Joints: ";
                for (int i = 0; i < 7; i++) {
            ...
            if (ruckig_result == ruckig::Result::Working || ruckig_result == ruckig::Result::Finished) {
                ...
                if (debug_counter % 1000 == 0) {
                    std::cout << "Ruckig error, using zero velocity for safety\n";
                }
            }
            ...
            // Capture a non-blocking snapshot of robot state and enqueue for async sending
            if (sender_) {
                LoggedState snap;
...
int main(int argc, char **argv)
{
    if (argc < 2 || argc > 4)
    {
        std::cerr << "Usage: " << argv[0] << " <robot-hostname> [receiver-ip] [bidexhand]" << std::endl;
        std::cerr << "  receiver-ip: optional IP address to send robot state to (default: 127.0.0.1)" << std::endl;
    ...
    int receiver_port = 9091; // Fixed port for now, or could parse from args
    bool bidexhand = false;

    // If provided, second argument is receiver_ip
    if (argc >= 3) {
        receiver_ip = argv[2];
    }
