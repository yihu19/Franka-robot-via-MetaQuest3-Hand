...
//   5. DOES NOT move to a preset initial pose; uses the current robot pose as start.
//
...
public:
    ...
    void run(const std::string& robot_ip,
        ...
        try {
            ...
            // ---- NO MOVE TO START POSE (Using current pose) ----
            std::cout << "Using current robot pose as initial configuration." << std::endl;
            ...
            for (int i = 0; i < 7; ++i) {
                ...
                // Using current pose as neutral pose for IK
                neutral_joint_pose_[i] = current_joint_angles[i];
            }
